...
const formatTextEditorCommand_1 = __webpack_require__(2);
function activate(context) {
    console.info('[vscode-json-stable-stringify] activated!');
    context.subscriptions.push(vscode.commands.registerTextEditorCommand('extension.jsonStableSort', formatTextEditorCommand_1.formatTextEditorCommand));
}
...
Object.defineProperty(exports, "__esModule", { value: true });
const vscode = __webpack_require__(1);
const stringifyResult_1 = __webpack_require__(3);
const stringify = __webpack_require__(4);
function formatTextEditorCommand(textEditor, edit) {
    let error = false;
    if (textEditor.selections.length === 1 && textEditor.selections[0].isEmpty) {
        ...
        let sorted = sortJson(text, textEditor.options, new vscode.Position(1, 1));
        error = !sorted.success;
        if (sorted.success) {
            let lastLine = textEditor.document.lineCount - 1;
            let textRange = new vscode.Range(0, textEditor.document.lineAt(0).range.start.character, lastLine, textEditor.document.lineAt(lastLine).range.end.character);
        ...
        for (var selectionIndex = 0; selectionIndex < textEditor.selections.length; selectionIndex++) {
            ...
            let sorted = sortJson(text, textEditor.options, selection.start);
            if (!sorted.success) {
                error = true;
                continue;
            }
...
exports.formatTextEditorCommand = formatTextEditorCommand;
function sortJson(original, editorOptions, start) {
    let opts = {
    ...
    return new stringifyResult_1.StringifyResult(success, sorted);
}
...
var json = typeof JSON !== 'undefined' ? JSON : __webpack_require__(5);
...
exports.stringify = __webpack_require__(7);
